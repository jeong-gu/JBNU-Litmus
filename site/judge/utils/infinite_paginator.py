...
class InfinitePage(collections.abc.Sequence):
    ...
    def page_range(self):
        ...
        # Add ... element if there is space in between leading_range and main_range.
        if result and result[-1] + 1 < main_range[0]:
            result.append(None)

        result += list(main_range)

        # Add ... element if there is space after main_range.
        if self.has_trailing:
            if main_range[-1] + 1 < self.trailing_range[0]:
                result.append(None)
            result += list(self.trailing_range)
