...
class ProblemDataCompiler(object):
    ...
    def compile(self):
        ...
        try:
            init = self.make_init()
            if init:
                init = self.parse_test_directory(init)
    ...
    def _extract_extention_zip(file_list):
        """
        같은 base 이름을 가진 파일 2개를 짝지어서 테스트케이스로 반환
        """
        ...
        for f in file_list:
            ...
            candidates[base].append(f)

        test_cases = []
        for base, files in candidates.items():
            if len(files) == 2:
                input_file, output_file = sorted(files)
    ...
    def parse_test_directory(self, init):

        try:

            if len(init['test_cases']) > 1:
                raise Exception('length is not `1`')

            if not(self.check_root_dir(init['test_cases'][0]['in'])):
                raise Exception('path should be root dir')

            prefix = init['test_cases'][0]['in']
            in_files = self.sort_files([
                f for f in self.files if re.match(rf'{re.escape(prefix)}.*\d{{1,4}}\.in$', f)
            ])

            out_files = self.sort_files([
                f for f in self.files if re.match(rf'{re.escape(prefix)}.*\d{{1,4}}\.out$', f)
            ])
            ...
            _test_cases = [
                {
                    'in': in_files[i],
                    'out': out_files[i],
                    'points': init['test_cases'][0]['points']
                }
                for i in range(len(in_files))
            ]

            init['test_cases'] = _test_cases
