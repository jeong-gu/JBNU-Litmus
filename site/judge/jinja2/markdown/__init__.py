...
@registry.filter
def markdown_text_only(value, style, math_engine=None):
    # First, convert markdown to HTML using the existing markdown function
    html_content = markdown(value, style, math_engine)
    
    # Use lxml to extract text from HTML
    try:
        ...
        # If parsing fails, return the original markdown content
        return Markup(value)
    
    return Markup(text_content)